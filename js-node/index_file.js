...
const PORT = process.env.PORT || 3000 // http服务端口
const SUB_PATH = process.env.SUB_PATH || '800' // 订阅路径
const config = {
  NAME: process.env.NAME || 'js-node', // 节点名称
  ...
  S5_PORT: process.env.S5_PORT || '', // Socks5端口(TCP)
  HY2_PORT: process.env.HY2_PORT || '', // Hy2端口(UDP)
  TUIC_PORT: process.env.TUIC_PORT || '', // Tuic端口(UDP)
  ANYTLS_PORT: process.env.ANYTLS_PORT || '', // AnyTLS端口(TCP)
  REALITY_PORT: process.env.REALITY_PORT || '', // Reality端口(TCP)
  ANYREALITY_PORT: process.env.ANYREALITY_PORT || '', // AnyReality端口(TCP)

  CHAT_ID: process.env.CHAT_ID || '', // telegram推送chat_id
  BOT_TOKEN: process.env.BOT_TOKEN || '', // telegram推送bot_token
  SHOW_LOG: process.env.SHOW_LOG || 'false' // 是否显示日志
}
...
async function main() {
  ...
  try {
    ...
    const downloadUrl = arch === 'amd64' ? 'https://amd64.eooce.com/sbsh' : 'https://arm64.eooce.com/sbsh'
